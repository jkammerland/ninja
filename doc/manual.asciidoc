...
`glob_watchfile`:: Used only on build statements that regenerate the
  ...
  a text file listing directories to watch for entry changes.
+
...
+
  Ninja records and compares watched directory mtimes between runs. A change
...
+
  ...
  manifest-listed source file inputs and can trigger an early manifest
