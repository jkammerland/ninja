...
`glob_watchfile`:: Used only on build statements that regenerate the
  manifest (for example, `build build.ninja: ...`).  If present, it names
  a text file listing extra directories to watch for entry changes.
+
  The watch file format is line-oriented UTF-8 text:
+
  * optional header line: `ninja_glob_watch_dirs_v1`
  * optional comments starting with `#`
  * one directory path per non-empty line
+
  If the first non-comment entry is a `ninja_glob_watch_dirs_v*` header with
  an unknown version, Ninja reports an error instead of treating it as a path.
+
  Ninja records and compares watched directory mtimes between runs. It
  uses directories listed by `glob_watchfile` as the authoritative watch set.
  Relative and absolute entries are treated uniformly, including paths under
  `builddir`.
  Entries that canonicalize to `.` are ignored to avoid self-triggering on
  Ninja bookkeeping files in the working directory.
  If `glob_watchfile` is omitted, Ninja infers watched directories from
  manifest leaf inputs.
+
  Cached mtimes are stored in `.ninja_glob_dirs` under `builddir` (or next
  to the manifest when `builddir` is unset). A detected directory-entry change
  triggers a manifest check/regeneration phase before building requested
  targets. Detection uses normal `stat()` mtime semantics.
+
  Inferred watching intentionally skips the working directory root (`.`) to
  avoid self-triggering on Ninja bookkeeping files.
...
`generator`:: if present, specifies that this rule is used to
  ...
  cleaned by default.
+
  Generator edges may list directory paths as explicit inputs. Ninja
  checks directory mtimes with the same `stat()` logic used for files,
  so adding or removing directory entries can make the generator edge
  dirty and trigger manifest checks/regeneration.
+
  For manifest regeneration edges, Ninja also tracks directories of
  manifest-listed leaf inputs and can trigger an early manifest
  check when source directory entries change.
